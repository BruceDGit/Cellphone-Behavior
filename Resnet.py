...
def build_resnet(input_shape, n_feature_maps, nb_classes):
    ...
    print('build conv_x')
    x1 = y
    conv_x = keras.layers.Conv2D(n_feature_maps * 2, 3, 1, padding='same')(x1)
    conv_x = keras.layers.BatchNormalization()(conv_x)
    conv_x = keras.layers.Activation('relu')(conv_x)

    print('build conv_y')
    conv_y = keras.layers.Conv2D(n_feature_maps * 2, 3, 1, padding='same')(conv_x)
    conv_y = keras.layers.BatchNormalization()(conv_y)
    conv_y = keras.layers.Activation('relu')(conv_y)
...
for fold, (train_index, valid_index) in enumerate(kfold.split(train_lstm, y)):
    ...
    model = build_resnet(train_lstm.shape[1:], 128, 19)
    ...
    model.load_weights(f'models/fold{fold}_resnet.h5')
